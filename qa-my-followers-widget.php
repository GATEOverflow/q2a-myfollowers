...
/*
	...
	File: qa-plugin/my-followers-widget/qa-my-followers-widget.php
...
class qa_my_followers_widget {

	const PLUGIN			= 'myfollowerswidget';
	const COUNT				= 'myfollowerswidget_count';
	const COUNT_DFL			= 10;
	const TITLE				= 'myfollowerswidget_title';
	const TITLE_DFL			= true;
	const DESC				= 'myfollowerswidget_desc';
	const DESC_DFL			= true;
	const AVATAR			= 'myfollowerswidget_avatar';
	const AVATAR_DFL		= true;
	const AVATAR_SIZE		= 'myfollowerswidget_avatar_size';
	const AVATAR_SIZE_DFL	= 50;
	const NAME				= 'myfollowerswidget_name';
	...
	const CUSTOM_CSSPATH_DFL= 'qa-plugin/my-followers-widget/qa-my-followers-widget-styles.css';	
	const SAVE_BUTTON		= 'myfollowerswidget_save_button';
	const DFL_BUTTON		= 'myfollowerswidget_dfl_button';
	const SAVED_MESSAGE		= 'myfollowerswidget_saved_message';
	const WIDGET_TITLE		= 'myfollowerswidget_widget_title';
	const WIDGET_DESC		= 'myfollowerswidget_widget_desc';
	const WIDGET_SETTING	= 'myfollowerswidget_widget_setting';

	const MYFOLLOWERS_PAGE_ENABLE	= 'myfollowerspage_enable';
	const MYFOLLOWERS_PAGE_REQUEST	= 'myfollowerspage_request';
	...
	function admin_form(&$qa_content) {
		...
		if (qa_clicked(self::SAVE_BUTTON)) {
			if (qa_post_text(self::COUNT.'_field') == '')
				$error = qa_lang_html(self::PLUGIN.'/'.self::COUNT.'_error');
			if (!is_numeric(qa_post_text(self::COUNT.'_field')))
				$error = qa_lang_html(self::PLUGIN.'/'.self::COUNT.'_error');
			if (qa_post_text(self::AVATAR_SIZE.'_field') == '')
				$error = qa_lang_html(self::PLUGIN.'/'.self::AVATAR_SIZE.'_error');
			if (!is_numeric(qa_post_text(self::AVATAR_SIZE.'_field')))
				$error = qa_lang_html(self::PLUGIN.'/'.self::AVATAR_SIZE.'_error');
			if ($error == '') {
	...
	function output_widget($region, $place, $themeobject, $template, $request, $qa_content){	
		...
		$users=qa_db_single_select($this->qa_db_user_my_followers_selectspec($userid));
		shuffle($users);
		
		$themeobject->output('<DIV CLASS="qa-my-followers-region-'.$region.'">');

		if(qa_opt(self::TITLE))
			$themeobject->output('<H2 CLASS="qa-my-followers-header">', qa_lang_html(self::PLUGIN.'/'.self::WIDGET_TITLE), '</H2>');
		if(qa_get_logged_in_level() >= QA_USER_LEVEL_ADMIN) {
			$themeobject->output('<P class="qa-my-followers-setting">');
			$themeobject->output(strtr(qa_lang_html(self::PLUGIN.'/' . self::WIDGET_SETTING),
				array(
					'^1' => '<A HREF="'.qa_path_html('admin/plugins', null, null, null, md5('widget/My Followers Widget')).'">',
					'^2' => '</A>')
				));
			$themeobject->output('</P>');
		}
		if(qa_opt(self::DESC)) {
			$themeobject->output('<P CLASS="qa-my-followers-desc">');
			$linkprefix = '';
			$linksufix = '';
			if(qa_opt(self::MYFOLLOWERS_PAGE_ENABLE) && qa_opt(self::MYFOLLOWERS_PAGE_REQUEST) != '') {
				$linkprefix = '<A HREF="'.qa_opt('site_url').'./'.qa_opt(self::MYFOLLOWERS_PAGE_REQUEST).'">';
				$linksufix = '</A>';
			}
			$themeobject->output($linkprefix, qa_lang_html_sub(self::PLUGIN.'/'.self::WIDGET_DESC, count($users)), $linksufix);
			$themeobject->output('</P>');
		}
		...
		if (count($users) >= 1) {
			...
			if(qa_opt(self::AVATAR))
				$class .= ' qa-my-followers-list-avatar-exist';
			else
				$class .= ' qa-my-followers-list-avatar-non';
			if(qa_opt(self::NAME))
				$class .= ' qa-my-followers-list-name-exist';
			else
				$class .= ' qa-my-followers-list-name-non';
			$themeobject->output('<UL CLASS="qa-my-followers-list'.$class.'">');
			foreach (array_slice($users,0,(int)qa_opt(self::COUNT)) as $user){
				...
				if(qa_opt(self::AVATAR))
					$themeobject->output('<SPAN CLASS="qa-my-followers-avatar">'.qa_get_user_avatar_html($user['flags'], $user['email'], $user['handle'], $user['avatarblobid'], $user['avatarwidth'], $user['avatarheight'], (int)qa_opt(self::AVATAR_SIZE), false).'</SPAN>');
				if(qa_opt(self::NAME)) {
					...
					elseif(qa_opt(self::NAME) == self::NAME_HANDLE)
						...
						if(isset($userprofile['name']) && @$userprofile['name'] != '')
							...
							$name = $user['handle'];
					if(qa_opt(self::AVATAR))
						$themeobject->output('<SPAN CLASS="qa-my-followers-name" STYLE="width:'.qa_opt(self::AVATAR_SIZE).'px;"><A HREF="'.qa_path_html('user/'.$user['handle']).'" CLASS="qa-user-link" TITLE="'.$name.'">'.$name.'</A></SPAN>');
					else
						$themeobject->output('<SPAN CLASS="qa-my-followers-name"><A HREF="'.qa_path_html('user/'.$user['handle']).'" CLASS="qa-user-link" TITLE="'.$name.'">'.$name.'</A></SPAN>');
				}
				$themeobject->output('</LI>');
			}
		...
		$themeobject->output('<DIV CLASS="qa-my-followers-clear"></DIV>');
	}

	function qa_db_user_my_followers_selectspec($userid)
	{
